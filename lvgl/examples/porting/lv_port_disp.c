 ...
 *  STATIC PROTOTYPES
 **********************/
static void disp_init(const char *lcd_dev_name);
...
void lv_port_disp_init(const char *lcd_dev_name)
{
    /*-------------------------
     * Initialize your display
     * -----------------------*/
    disp_init(lcd_dev_name);
    ...
    static lv_color_t buf1_1[LV_HOR_RES_MAX * 120];                      /*A buffer for 10 rows*/
    lv_disp_buf_init(&disp_buf_1, buf1_1, NULL, LV_HOR_RES_MAX * 120);   /*Initialize the display buffer*/
...
{
    /*You code here*/
    lcd_device = rt_device_find(lcd_dev_name);

    if (!lcd_device)
    {
        LOG_E("find %s failed!", lcd_dev_name);
        return;
    }
